 ...
 * FloatingInput
 *
 * Preserve deletion/backspace behavior and collapse repeated spaces on insert.
 */
const FloatingInput: React.FC<{
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If user deleted accept as-is so backspace works
    ...
    // On insertion, collapse repeated spaces to a single space and preserve trailing space
...
/**
 * New FloatingDateParts: matches SignUp's date selector design and behavior.
 * - Month dropdown (custom)
 * - Day input (2 digits)
 * - Year input (4 digits, selectable pattern)
 * - Same box sizing/layout as the SignUp implementation
 */
const FloatingDateParts: React.FC<{
  ...
  // keep years but we don't render huge lists—users type year. In SignUp the year was an input; here keep input behavior.
  ...
  // sync incoming prop -> local parts
  useEffect(() => {
  ...
  // when parts change, validate and call onChange with ISO y-m-d if valid
  useEffect(() => {
  ...
  // adjust day when month/year changes so it remains valid
  useEffect(() => {
    if (!day) return;
        ...
        <div>
          <input
            ...
            onChange={(e) => {
              ...
              setDay(v.padStart(v.length, '0').slice(0, 2) || v); // preserve partial
            }}
            inputMode="numeric"
            maxLength={2}
            placeholder="DD"
...
const ClientInfoStep: React.FC<ClientInfoStepProps> = ({ formData, onUpdate, onUpdateField, onNext, onBack }) => {
  ...
  return (
    // Keep extra bottom padding so the fixed footer never overlaps content
